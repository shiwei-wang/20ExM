...
if needToMask
    ...
    blurred.Moving = (blurred.Moving - min(blurred.Moving(:)))/(max(blurred.Moving(:))-min(blurred.Moving(:)));
    blurred.Static = (blurred.Static - min(blurred.Static(:)))/(max(blurred.Static(:))-min(blurred.Static(:)));
...
if needToDoSections
    sectionInfo.width = imgSize/2;
    if doNotSection
        sectionInfo.width = imgSize;
    end
    ...
    for i = 1:size(sections,1)
        for j = 1:size(sections,2)
            ...
            sections{i,j}.static.mask = static.maskPadded([1:sectionInfo.width+2*sectionInfo.overlap]+sectionInfo.width*(i-1),[1:sectionInfo.width+2*sectionInfo.overlap]+sectionInfo.width*(j-1));
