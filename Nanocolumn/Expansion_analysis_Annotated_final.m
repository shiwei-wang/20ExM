...
imsizerow = size(d0r,1); imsizecol = size(d0r,2);
d0m = zeros(imsizerow, imsizecol, 3);

%Adjust contrast for each channel
t = max(0,d0rmax-0); t = t/300; d0m(:,:,1) = t(:,:); %Red channel
%t = max(0,d0gmax-150); t = t/3200; d0m(:,:,2) = t(:,:); %Green channel
t = max(0,d0bmax-0); t = t/200; d0m(:,:,3) = t(:,:); %Blue channel
...
for ii = 1:length(list)
    file1 = fullfile([list(ii).name]);
    test = loadtiff(which(file1));
    numberofimgs = size(test,3)/3;
    t = 1:numberofimgs;
    A = test(:,:,t*2);  %blue channel
    B = test(:,:,t*2-1); %red channel
...
flag = 0;
cc = get_corr_3dMatrix_final(B1, B1, pixel, rmax, xyzshift, flag); %Enter same variable twice for autocorr
temp = [kkk, ii, 0, cc];
res = [res;temp];
end
end
...
step = 5;   %Step size distance for the analysis, in nm. Ideally, this should be in
...
distance = [20,180];    %Min and max distance allowed for xyz shift, in nm
flag = 0;
[Raa,Rab,Rba,Rbb] = get_enrichment_3dMatrix_final(A1, B1, pixel, rmax, xyzshift, distance, step, flag);
aatemp = [kkk,ii,0,Raa'];
abtemp = [kkk,ii,0,Rab'];
batemp = [kkk,ii,0,Rba'];
bbtemp = [kkk,ii,0,Rbb'];
